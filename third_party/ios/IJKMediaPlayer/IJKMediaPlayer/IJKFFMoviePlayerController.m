...
#pragma mark Record
- (int)startRecordWithPath:(NSString *)path
{
    if (!_mediaPlayer || path.length == 0)
        return -1;
    return ijkmp_record_start(_mediaPlayer, path.UTF8String);
}
...
- (int)stopRecord
{
    if (!_mediaPlayer)
        return -1;
    return ijkmp_record_stop(_mediaPlayer);
}
