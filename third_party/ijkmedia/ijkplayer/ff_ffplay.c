...
//      of the seek_pos/seek_rel variables
        ...
        if (ffp->record_enabled && ffp->record_ctx && pkt->stream_index >= 0
                && pkt->stream_index < (int)ic->nb_streams) {
            ffp_record_write_packet_l(ffp, pkt, ic);
        }
...
FFPlayer *ffp_create()
{
    ...
    ffp->record_mutex = SDL_CreateMutex();
...
void ffp_destroy(FFPlayer *ffp)
{
    ...
    if (ffp->record_mutex) {
        SDL_DestroyMutex(ffp->record_mutex);
        ffp->record_mutex = NULL;
    }
...
static void ffp_record_write_packet_l(FFPlayer *ffp, AVPacket *pkt, AVFormatContext *ic)
{
    ...
    if (!ffp->record_ctx || !ffp->record_stream_map) {
        SDL_UnlockMutex(ffp->record_mutex);
        return;
    }

    int rindex = ffp->record_stream_map[pkt->stream_index];
    if (rindex < 0) {
        SDL_UnlockMutex(ffp->record_mutex);
        return;
    }
    ...
    AVStream *ost = ffp->record_ctx->streams[rindex];
    if (!ist || !ost) {
        SDL_UnlockMutex(ffp->record_mutex);
        return;
    }
    ...
    /* keep dts/pts monotonic: live streams can jump back on reconnect */
    if (ffp->record_last_dts != AV_NOPTS_VALUE && opkt.dts != AV_NOPTS_VALUE) {
        if (opkt.dts <= ffp->record_last_dts)
            opkt.dts = ffp->record_last_dts + 1;
        if (opkt.pts != AV_NOPTS_VALUE) {
            if (opkt.pts <= ffp->record_last_pts)
                opkt.pts = ffp->record_last_pts + 1;
            else if (opkt.pts < opkt.dts)
                opkt.pts = opkt.dts;
        }
    }
    ffp->record_last_dts = opkt.dts;
    ffp->record_last_pts = opkt.pts;
...
int ffp_record_start_l(FFPlayer *ffp, const char *path)
{
    ...
    ffp->record_stream_map = (int *)av_mallocz(sizeof(int) * is->ic->nb_streams);
    if (!ffp->record_stream_map) {
        ret = -1;
        goto fail;
    }
    for (i = 0; i < (int)is->ic->nb_streams; i++)
        ffp->record_stream_map[i] = -1;

    ffp->record_mapped_streams = 0;
    for (i = 0; i < (int)is->ic->nb_streams; i++) {
        AVStream *ist = is->ic->streams[i];
        if (!ist || !ist->codecpar)
            continue;
    ...
    if (ffp->record_mapped_streams <= 0) {
        ret = -1;
        goto fail;
    }
    ...
    SDL_UnlockMutex(ffp->record_mutex);
    av_log(ffp, AV_LOG_INFO, "record: start path=%s streams=%d\n", path, ffp->record_mapped_streams);
    return 0;
...
int ffp_record_stop_l(FFPlayer *ffp)
{
    if (!ffp)
        return -1;
    ...
    avformat_free_context(oc);
    av_log(ffp, AV_LOG_INFO, "record: stopped\n");
    return 0;
}
