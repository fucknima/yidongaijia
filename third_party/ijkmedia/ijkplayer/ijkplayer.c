...
int ijkmp_record_start(IjkMediaPlayer *mp, const char *path)
{
    assert(mp);
    assert(path);

    MPTRACE("%s(%s)\n", __func__, path);
    int ret = ffp_record_start_l(mp->ffplayer, path);
    MPTRACE("%s()=%d\n", __func__, ret);
    return ret;
}

int ijkmp_record_stop(IjkMediaPlayer *mp)
{
    assert(mp);

    MPTRACE("%s()\n", __func__);
    int ret = ffp_record_stop_l(mp->ffplayer);
    MPTRACE("%s()=%d\n", __func__, ret);
    return ret;
}
