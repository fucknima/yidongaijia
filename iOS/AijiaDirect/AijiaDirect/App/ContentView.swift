...
private struct ReplayControls: View {
    ...
    private var controlsContent: some View {
        VStack(spacing: presentation == .fullscreen ? 6 : 8) {
            ...
            if isEditing, let recording = model.replayRecording {
                Text("预览时间：\(formatClock(recording.startDate.addingTimeInterval(TimeInterval(previewSecond))))")
                    .font(.caption.monospacedDigit())
                    .foregroundStyle(secondaryForegroundColor)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }
    ...
    private var previewSecond: Int64 {
        guard isEditing else { return model.replayCurrentSecond }
        return max(0, min(model.replayDurationSecond, Int64((sliderValue * Double(model.replayDurationSecond)).rounded())))
    }
        ...
        NavigationView {
            Form {
                Section("自由选择下载范围") {
                    ...
                    Slider(value: $startOffset, in: 0...duration, step: 1)
                        .onChange(of: startOffset) { value in
                            let maximum = max(0, endOffset - 1)
                            if value > maximum { startOffset = maximum }
                        }
                    rangeLabel("结束", value: formatted(endOffset))
                    Slider(value: $endOffset, in: 0...duration, step: 1)
                        .onChange(of: endOffset) { value in
                            let minimum = min(duration, startOffset + 1)
                            if value < minimum { endOffset = minimum }
                        }
